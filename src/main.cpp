...
const string strMessageMagic = "FIFAWCCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // FIFAWCCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // FIFAWCCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
//const int DAILY_BLOCKCOUNT =  0;
//const int YEARLY_BLOCKCOUNT =  0; 
...
// FIFAWCCoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes FIFAWCCoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // FIFAWCCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // FIFAWCCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // FIFAWCCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // FIFAWCCoin: track money supply and mint amount info
...
// FIFAWCCoin: total coin age spent in transaction, in the unit of coin-days.
...
// FIFAWCCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // FIFAWCCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // FIFAWCCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // FIFAWCCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // FIFAWCCoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // FIFAWCCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // FIFAWCCoin: check proof-of-stake
    ...
    // FIFAWCCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // FIFAWCCoin: getblocks may not obtain the ancestor block rejected
    ...
    // FIFAWCCoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "FIFAWCCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "World Cup 2014";
        CTransaction txNew;
        txNew.nTime = 1402420416;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1402420428;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 10709840;
        if(fTestNet)
        {
            block.nNonce   = 10709840;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xe030ecd2162d03d4eead332e259ca09943074ab1a3e451169f176e182ea162b5"));
        block.print();
        ...
        // FIFAWCCoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // FIFAWCCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // FIFAWCCoin: tell downloading node about the latest block if it's
